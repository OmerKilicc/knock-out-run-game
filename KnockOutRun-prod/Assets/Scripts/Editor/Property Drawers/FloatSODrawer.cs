...
{
    [CustomPropertyDrawer(typeof(FloatSO))]
    public class FloatSODrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            Rect valRect = new Rect(EditorGUIUtility.currentViewWidth - 70, position.y, 60, position.height);

            EditorGUI.ObjectField(objrect, property);

            SerializedProperty myProp = SO.FindProperty("_value");
            myProp.floatValue = EditorGUI.FloatField(valRect, myProp.floatValue);
